...
class AppPage extends React.PureComponent<AppPageReduxProps & AppPageReduxDispatchProps & AppPageComponentProps, AppPageState> {
  props: AppPageReduxProps &
    AppPageReduxDispatchProps &
  ...
  private persistTeam = async (tmpTeam) => {
    ...
    const response = await fetch(`/rest/app/${this.getApp().id}/team`, {
      method: "POST",
      credentials: "include",
      body: JSON.stringify({
  ...
  createChannel = async () => {
    ...
    try {
      const newChannel = await (
        await fetch(`/rest/app/${app.id}/channel`, {
          method: "POST",
          credentials: "include",
          body: JSON.stringify({
...
autoUpdater.setFeedURL({
  ...
  // More config
              ...
              <div
                ...
                style={{ fontSize: 14, fontFamily: '"SFMono-Medium", "SF Mono", "Segoe UI Mono", "Roboto Mono", "Ubuntu Mono", Menlo, Courier, monospace' }}
              >
...
sudo apt-get install <package-name>`}
    ...
    return channels.concat({
      ...
      content: (
        <div style={{ flex: 1 }}>
          ...
          <div style={{ height: 8 }} />
          <AkButton appearance="primary" onClick={this.createChannel} isDisabled={this.props.hasPendingMigration}>
            Create
          </AkButton>
        </div>
      ),
  ...
  resetToken = async () => {
    ...
    const newApp = await (
      await fetch(`/rest/app/${app.id}/refresh_token`, {
        credentials: "include",
        method: "POST",
      })
    ).json()
  ...
  private onNewIcon = async (files: FileList) => {
    const app = this.getApp()
    if (app && files && files.length && files[0] && files[0].type === "image/png") {
      ...
      alert("Although the image has been uploaded, please be aware that on " + "some Nucleus configuration the image may not update for 15-30 " + "minutes due to the use of a CDN")
    }
        ...
        {!this.state.loading && this.props.apps ? (
          app ? (
            <div>
              ...
              <div style={{ display: "flex" }}>
                <div className={styles.tokenContainer}>
                  <HelpHeader
                    ...
                    help={
                      <p>
                        This is the API token to use in the code examples provided in the Channel's section. Make sure you don't share with anyone or hard code this value into
                        Version Control.
                      </p>
                    }
                    position="right top"
                  ...
                  <div style={{ height: 8 }} />
                  <AkButton appearance="danger" onClick={this.resetToken} isDisabled={this.props.hasPendingMigration}>
                    Reset Token
                  </AkButton>
                </div>
                ...
                <HelpHeader
                  ...
                  help={
                    <div>
                      ...
                      <p>
                        For reference your identifier is <b>"{this.props.user.id}"</b>
                      </p>
                    </div>
                  }
                />
              ...
              <WebHookManagement app={app} apps={this.props.apps} setApps={this.props.setApps} hasPendingMigration={this.props.hasPendingMigration} />
            </div>
          ) : (
